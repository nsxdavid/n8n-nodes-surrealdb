...
### Fixed
- **Query Operations**: Fixed "Received tool input did not match expected schema" error when executing queries without parameters
  - The `parameters` field in Execute Query and Build Select Query operations is now correctly treated as optional
  - Empty parameters (`{}`) no longer cause validation errors
  - Added proper handling for undefined or empty parameter values
